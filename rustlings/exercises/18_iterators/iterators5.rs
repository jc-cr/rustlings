...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
    ...
    // filter iter for those that match desired progress val
    // counts the iterators in this filtered
    map.values().filter(|&&v| v == value).count()
    //.values() gives &Progress references
    //.filter() gives you a reference to each item, so you get &&Progress
    //&&v unpacks both layers of references so v is a Progress value
}
...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    // get map from collection
    // filter for those with expected progress
    // return sum
